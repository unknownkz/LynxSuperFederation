...
async def user_is_ban_protected(user_id: int, message):
    status = False
    if message.is_private or user_id in (U_ABSURD):
        return True
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in lynx_client.iter_participants(
        ...
        if user_id == user.id or user_id in SD_ID:
            status = True
            break
    return status
...
async def lynx_is_admin(chat_id: int):
    status = False
    kz = await lynx_client.get_me()
    async for user in lynx_client.iter_participants(
        chat_id,
        filter=ChannelParticipantsAdmins,
    ):
...
async def is_user_in_chat(chat_id: int, user_id: int):
    status = False
    async for user in lynx_client.iter_participants(chat_id):
        if user_id == user.id:
            status = True
            break
    return status
