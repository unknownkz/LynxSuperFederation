...
def info(update: Update, context: CallbackContext):
    ...
    if disaster_level_present:
        text += ' [<a href="https://t.me/LynxSuperFederation">?</a>]'.format(bot.username)

    try:
        user_member = chat.get_member(user.id)
        if user_member.status == "administrator":
            result = requests.post(
                f"https://api.telegram.org/bot{TGB_TOKEN}/getChatMember?chat_id={chat.id}&user_id={user.id}"
            )
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 1448477501] and (user_id in DEV_ID):
