...
def callback_button(update: Update, context: CallbackContext):
    ...
    if query_type == "db_leave_chat":
        if query.from_user.id in DEV_ID:
        ...
        if query.from_user.id in DEV_ID:
            ...
            reply = "Cleaning database...\nOutput: {} gbanned users and {} chats.".format(
                invalid_gban_count,
                invalid_chat_count,
            )
...
DB_CLEANUP_HANDLER = CommandHandler("cleaning", dbcleanup, run_async=True)
