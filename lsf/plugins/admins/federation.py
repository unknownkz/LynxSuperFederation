...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in SD_ID:
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in SD_ID:
        message.reply_text("My Contributor cannot be fed banned!")
        return

    if int(user_id) in TIGERS_ID:
        message.reply_text("My Brother cannot be fed banned!")
        return

    if int(user_id) in WHITELIST_ID:
        message.reply_text("My Friends cannot be fed banned!")
        return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "lynx_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="lynx_fbanned_users.json",
        ...
        if args[0] == "csv":
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "lynx_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="lynx_fbanned_users.csv",
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in SD_ID:
                        failed += 1
                        continue
                    if int(import_userid) in TIGERS_ID:
                        failed += 1
                        continue
                    if int(import_userid) in WHITELIST_ID:
                        failed += 1
                        continue
...
__handlers__ = [NEW_FED_HANDLER, DEL_FED_HANDLER, RENAME_FED, JOIN_FED_HANDLER, PROMOTE_FED_HANDLER, DEMOTE_FED_HANDLER, INFO_FED_HANDLER, BAN_FED_HANDLER, UN_BAN_FED_HANDLER, FED_BROADCAST_HANDLER, FED_SET_RULES_HANDLER, FED_GET_RULES_HANDLER, FED_CHAT_HANDLER, FED_ADMIN_HANDLER, FED_USERBAN_HANDLER, FED_NOTIF_HANDLER, FED_CHATLIST_HANDLER, FEDSTAT_USER, SET_FED_LOG, UNSET_FED_LOG, SUBS_FED, UNSUBS_FED, MY_SUB_FED, MY_FEDS_LIST, DELETEBTN_FED_HANDLER, FED_OWNER_HELP_HANDLER, FED_ADMIN_HELP_HANDLER, FED_USER_HELP_HANDLER]
