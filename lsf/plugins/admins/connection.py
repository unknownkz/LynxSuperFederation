...
from ... import dispatcher, SD_ID, DEV_ID
from ...database import connection_sql as sql
...
def connected(bot: Bot, update: Update, chat, user_id, need_admin=True):
    ...
    if chat.type == chat.PRIVATE and sql.get_connected_chat(user_id):
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SD_ID) or (user.id in DEV_ID):
            if need_admin is True:
                if getstatusadmin.status in ("administrator", "creator") or user_id in SD_ID or user.id in DEV_ID:
                    return conn_id
                else:
                    send_message(
                        update.effective_message,
...
3.) Get invite link of chat.
4.) Set and control AntiFlood settings.
5.) Set and control Blacklist settings.
6.) Set Locks and Unlocks in chat.
7.) Enable and Disable commands in chat.
8.) Export and Imports of chat backup.
9.) More in future!
"""
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SD_ID):
            ...
            if connection_status:
                ...
                query.message.edit_text(
                    "Successfully connected to *{}*. \nHit /helpconnect to check available commands.".format(chat_name),
                    parse_mode=ParseMode.MARKDOWN,
                )
                sql.add_history_conn(user.id, str(conn_chat.id), chat_name)
            else:
