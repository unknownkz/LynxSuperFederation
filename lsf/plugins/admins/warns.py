...
from ...handlers.extraction import extract_text, extract_user, extract_user_and_text
from ...handlers.misc import split_message
from ...handlers.string_handling import split_quotes
from ...handlers.valid import bot_admin, can_restrict, is_user_admin, user_admin, user_admin_no_reply
...
def warn(
    ...
    if user.id in SD_ID:
        if warner:
            message.reply_text("My Team cant be warned.")
        else:
            message.reply_text(
                "Tiger triggered an auto warn filter!\n I won't warn my team but they should avoid abusing this.",
            )
        return

    if user.id in SUPPORT_ID:
        if warner:
            message.reply_text("My Team Support disasters are warn immune.")
        else:
            message.reply_text(
                "My Team Support Disaster triggered an auto warn filter!\nI won't warn support but they should avoid abusing this.",
            )
        return
