import importlib, re
from lsf import dispatcher, ALLOW_EXCL, LOGGER
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, ParseMode, Update
from lsf.handlers.misc import paginate_plugins
...
I'm a management group focused on the Lynx Federation! Have a look at the following for an idea of some of the things I can help you with. """.format(
...
def admin_help_button(update, context):
    ...
    mod_match = re.match(r"admin_plugins\((.+?)\)", query.data)
...
def user_help_button(update, context):
    query = update.callback_query
    mod_match = re.match(r"user_plugins\((.+?)\)", query.data)
...
def tools_help_button(update, context):
    query = update.callback_query
    mod_match = re.match(r"tools_plugins\((.+?)\)", query.data)
...
def help_button(update, context):
    query = update.callback_query
    mod_match = re.match(r"help_plugins\((.+?)\)", query.data)
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            update.effective_message.reply_text(
                f"Contact me in PM to get help of {plugins.capitalize()}",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
...
def settings_button(update: Update, context: CallbackContext):
    ...
    bot = context.bot
    mod_match = re.match(r"stngs_plugins\((.+?),(.+?)\)", query.data)
